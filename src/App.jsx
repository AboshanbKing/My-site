...
function App() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="live-section section-wrap" id="live" data-reveal><div className="section-heading"><div><span className="section-kicker">{t.live.kicker}</span><h2>{t.live.title} <span>KICK</span></h2></div><div className="section-status"><span className={channel.isLive ? 'status-dot live' : 'status-dot'} /> {!channel.apiAvailable ? t.live.unavailable : channel.isLive ? t.live.liveNow : t.live.offline}</div></div><div className="live-layout"><div className="stream-frame">{channel.isLive ? <iframe src={`${KICK_URL}?embed=true`} title="Aboshanb King live stream" allowFullScreen /> : <div className="offline-state"><div className="offline-orb"><Radio size={35} /></div><span className="offline-label">{channel.apiAvailable ? t.live.offline : t.live.unavailable}</span><h3>{channel.apiAvailable ? t.live.nextSession : t.live.unavailable}</h3><p>{t.live.followNotice}</p><button className="button button-primary" type="button" onClick={() => openExternal(KICK_URL)}>{t.hero.followKick} <ExternalLink size={16} /></button></div>}</div><StreamStatusCard channel={channel} language={language} /></div></section>
